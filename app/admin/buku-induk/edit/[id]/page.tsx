...
export default function BukuIndukDashboardPage() {
  ...
  // 1. Ambil Data Real-Time Dari Database (Via API Route)
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        setLoading(true);
        const res = await fetch("/api/admin/buku-induk/dashboard-summary");
        if (res.ok) {
  ...
  const handleHapusDosen = async (id: number, nama: string) => {
    if (confirm(`Apakah Anda yakin ingin menghapus data ${nama} dari Buku Induk?`)) {
      try {
        const res = await fetch(`/api/buku-induk/${id}`, { method: "DELETE" });
        if (res.ok) {
  ...
  const dataTerfilter = daftarDosen.filter((dosen: any) => {
    const namaDosen = String(dosen.nama_lengkap || dosen.nama || "").toLowerCase();
    const nipDosen = String(dosen.nip || "").toLowerCase();
    const jurusanDosen = String(dosen.jurusan || dosen.prodi || "").toLowerCase();
    
    const statusKuliah = String(dosen.status_kuliah || dosen.statusKuliah || "Aktif").toLowerCase();
    const jenisStudi = String(dosen.jenis_pengajuan_studi || dosen.jenisPengajuanStudi || "").toLowerCase();
    const jenjangStudi = String(dosen.jenjang || dosen.jenjangStudi || "").toLowerCase();
    const jabatanDosen = String(dosen.jabatan || dosen.jabatanFungsional || "").toLowerCase();

    // A. Logika Pencarian Aktif
    const matchesSearch = 
      searchTerm.trim() === "" || 
      namaDosen.includes(searchTerm.toLowerCase()) ||
      nipDosen.includes(searchTerm.toLowerCase()) ||
      jurusanDosen.includes(searchTerm.toLowerCase());
      
    // B. Cocokkan dengan 5 Dropdown Jurusan Resmi Polines
    const matchesJurusan = 
      filterJurusan === "Semua Jurusan" || 
      jurusanDosen.includes(filterJurusan.toLowerCase());
    
    // C. Cocokkan dengan Dropdown Status Kelayakan Kuliah
    const matchesStatus = 
      filterStatus === "Semua Status" || 
      statusKuliah === filterStatus.toLowerCase();
    ...
    const matchesTab = 
      ...
      (activeTab === "Doktor" && jenjangStudi.includes("s3")) ||
      (activeTab === "Profesor" && jabatanDosen.includes("profesor"));
                  ...
                  {loading ? (
                    ...
                    dataTerfilter.map((dosen: any, i) => (
                      <tr key={dosen.id} className="hover:bg-slate-50/50 transition-all">
                        ...
                        <td className="p-4 text-[#0085FF] font-black cursor-pointer hover:underline" onClick={() => router.push(`/admin/buku-induk/detail/${dosen.id}`)}>{dosen.nama_lengkap}</td>
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-1.5">
                            {/* DI SINI PERBAIKANNYA: Tombol Eye sekarang mengarah ke modul rute DETAIL */}
                            <button 
                              onClick={() => router.push(`/admin/buku-induk/detail/${dosen.id}`)} 
                              className="p-1.5 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-lg transition-colors"
                              title="Lihat Detail Profil"
                            >
                              <Eye size={14} />
                            </button>
                            
                            {/* Tombol Edit tetap ke rute EDIT form input */}
                            <button 
                              onClick={() => router.push(`/admin/buku-induk/edit/${dosen.id}`)} 
                              className="p-1.5 bg-blue-50 text-[#0085FF] rounded-lg"
                              title="Ubah Data"
                            >
                              <Edit size={14} />
                            </button>
                            
                            <button onClick={() => handleHapusDosen(dosen.id, dosen.nama_lengkap)} className="p-1.5 bg-red-50 text-red-500 rounded-lg" title="Hapus Data"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
