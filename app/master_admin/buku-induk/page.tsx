...
export default function BukuIndukDashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        setLoading(true);
        const res = await fetch("/api/master_admin/buku-induk");
        if (res.ok) {
  ...
  const handleHapusDosen = async (id: number, nama: string) => {
    if (confirm(`Apakah Anda yakin ingin menghapus data ${nama} dari Buku Induk?`)) {
      try {
        const res = await fetch(`/api/master_admin/buku-induk?id=${id}`, {
          method: "DELETE"
        });
    ...
    <div className="p-6 bg-[#F4F7FE] min-h-screen text-[#1F1F1F] font-sans selection:bg-[#0085FF]/20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button onClick={() => router.push("/master_admin/buku-induk/tambah")} className="flex items-center gap-2 bg-[#0085FF] hover:bg-[#006ACC] text-white px-4 py-2.5 rounded-xl text-xs font-black shadow-md shadow-blue-500/10 transition-all ml-auto md:ml-0">
                  ...
                  {loading ? (
                    ...
                    dataTerfilter.map((dosen: any, i) => (
                      <tr key={dosen.id} className="hover:bg-slate-50/50 transition-all">
                        ...
                        <td className="p-4 text-[#0085FF] font-black cursor-pointer hover:underline" onClick={() => router.push(`/master_admin/buku-induk/edit/${dosen.id}`)}>{dosen.nama_lengkap}</td>
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => router.push(`/master_admin/buku-induk/edit/${dosen.id}`)} className="p-1.5 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-lg transition-colors"><Eye size={14} /></button>
                            <button onClick={() => router.push(`/master_admin/buku-induk/edit/${dosen.id}`)} className="p-1.5 bg-blue-50 text-[#0085FF] rounded-lg"><Edit size={14} /></button>
                    ...
                    lulusanTerbaru.map((lulus: any) => (
                      <tr key={lulus.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="py-3 text-center">
                          <button onClick={() => router.push(`/master_admin/buku-induk/edit/${lulus.id}`)} className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-[#0085FF] font-black text-[11px] rounded-xl transition-all">
