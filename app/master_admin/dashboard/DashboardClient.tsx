// app/master_admin/dashboard/DashboardClient.tsx
"use client";
...
const RechartsBar = dynamic(() => import("recharts").then(mod => {
  ...
  return function ChartBar({ data }: { data: any[] }) {
    return (
          ...
          {data.urgentTasks.length > 0 ? (
            <div className="space-y-3">
              {data.urgentTasks.map((task: any) => (
                <Link href={`/master_admin/verifikasi-pengajuan/${task.pengajuan_id}`} key={task.id} className="group flex justify-between items-center p-3 border border-slate-100 bg-white hover:border-blue-300 hover:shadow-sm rounded-xl transition-all cursor-pointer">
                  <div>
                ...
                {data.lewatBatas.map((item: any) => (
                  <tr key={item.id} className="hover:bg-rose-50/30 transition">
                    ...
                    <td className="px-4 py-3 text-center">
                      <Link href={`/master_admin/verifikasi-pengajuan/${item.pengajuan_id}`} className="text-xs font-bold text-blue-600 hover:underline">Tinjau</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
