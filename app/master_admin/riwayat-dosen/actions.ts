...
async function evaluateKhs(khsId: number, keputusan: 'diterima' | 'ditolak', catatan: string) {
  ...
  revalidatePath('/master_admin/riwayat-dosen');
  if (khs?.pengajuan_studi?.user_id) {
    const uid = khs.pengajuan_studi.user_id;
    revalidatePath(`/master_admin/riwayat-dosen/${uid}/khs`);
    revalidatePath(`/master_admin/riwayat-dosen/${uid}/khs/${khsId}`);
  }
}
...
export async function uploadBuktiTransfer(formData: FormData) {
  ...
  revalidatePath('/master_admin/riwayat-dosen');
  revalidatePath(`/master_admin/riwayat-dosen/${idDosen}/keuangan`);
  revalidatePath(`/master_admin/riwayat-dosen/${idDosen}/keuangan/${keuanganId}`);
}
...
async function evaluateKeuangan(keuanganId: number, keputusan: 'dicairkan' | 'ditolak', catatan: string) {
  ...
  if (reimbursement?.pengajuan_studi?.user_id) {
    ...
    revalidatePath(`/master_admin/riwayat-dosen/${uid}/keuangan`);
    revalidatePath(`/master_admin/riwayat-dosen/${uid}/keuangan/${keuanganId}`);
  }
}
...
export async function addManualDokumen(formData: FormData) {
  ...
  if (dokumen.pengajuan_studi?.user_id) {
    ...
    revalidatePath(`/master_admin/riwayat-dosen/${uid}/dokumen`);
  }
}
